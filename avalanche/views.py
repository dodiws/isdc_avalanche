...
def getAvalancheForecast(request, filterLock, flag, code, includes=[], excludes=[]):
    response = dict_ext()
    ...
    response['avalanchforecast'] = getRawAvalancheForecast(request, filterLock, flag, code)
    ...
    response.path('avalanchforecast')['pop_forecast_percent'] = {k:round(div_by_zero_is_zero(v, response['baseline']['pop_total'])*100, 0) for k,v in response['avalanchforecast']['pop_likelihood'].items()}
    response.path('avalanchforecast')['pop_forecast_total_percent'] = sum(response['avalanchforecast']['pop_forecast_percent'].values())
    ...
    if 'date' not in request.GET:
        ...
        response.path('avalanchforecast')['lc_child']=data
...
def getRawAvalancheForecast(request, filterLock, flag, code):
    ...
    for i,j in {'pop':'pop','building':'buildings'}.items():
        ...
        response[i+'_likelihood'] = {AVA_LIKELIHOOD_TYPES[v]:temp.get(int(k)) or 0 for k,v in AVA_LIKELIHOOD_INDEX.items()}
...
def dashboard_avalancheforecast(request, filterLock, flag, code, includes=[], excludes=[]):
	response = dict_ext(getCommonUse(request, flag, code))
	response['source'] = getAvalancheForecast(request, filterLock, flag, code)
